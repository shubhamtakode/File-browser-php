...
$_CONFIG['hidden_files'] = array(".ftpquota",".ftpquota~", "index.php", "index.php~", ".htaccess", ".htpasswd",".htaccess~", ".htpasswd~","web.xml",".jsp","sitemap.xml",".xml",".html~",".jsp~",".xml~",".htm~",".php",".php~","access.php","admin.php","login.php","log.txt");
...
$_CONFIG['log_file'] = "";
$_CONFIG['starting_dir'] = "./hjbvshvfdf78e59758rubnggvyuyxhcvbuxhyvb/";
$_CONFIG['basedir'] = "";
...
class ImageServer
{
	public static function showImage()
	{
		...
		{
			if(strlen($_GET['thumb']) > 0 && DownloadExplorer::getConfig('thumbnails') == true)
			{
	...
	{
		if(is_int(DownloadExplorer::getConfig('thumbnails_width')))
			$max_width = DownloadExplorer::getConfig('thumbnails_width');
		else
			$max_width = 200;
		
		if(is_int(DownloadExplorer::getConfig('thumbnails_height')))
			$max_height = DownloadExplorer::getConfig('thumbnails_height');
		else
			$max_height = 200;
...
class File
{
	...
	public static function getFileSize($file)
	{
		...
		if (DownloadExplorer::getConfig("large_files") == true || !$sizeInBytes || $sizeInBytes < 0) {
...
class Location
{
	...
	{
		return (strlen(DownloadExplorer::getConfig('basedir')) > 0?DownloadExplorer::getConfig('basedir'):dirname($_SERVER['SCRIPT_FILENAME']))."/".$this->getDir(true, false, false, 0);
	}
	...
	function init()
	{
		...
		{
			$this->path = $this->splitPath(DownloadExplorer::getConfig('starting_dir'));
		}
		else
		{
...
class DownloadExplorer
{
	...
	function init()
	{
		...
		if(isset($_GET['lang']) && isset($_TRANSLATIONS[$_GET['lang']]))
			...
			$this->lang = DownloadExplorer::getConfig("lang");
		
		$this->mobile = false;
		if(DownloadExplorer::getConfig("mobile_enabled") == true)
		{
			if((DownloadExplorer::getConfig("mobile_default") == true || isset($_GET['m'])) && !isset($_GET['s']))
				$this->mobile = true;
		}
		
		$this->logging = false;
		if(DownloadExplorer::getConfig("log_file") != null && strlen(DownloadExplorer::getConfig("log_file")) > 0)
			$this->logging = true;
	}
	...
	function readDir()
	{
		...
		if($open_dir = @opendir($this->location->getFullPath()))
		{
			...
			while ($object = readdir($open_dir))
			{
				if($object != "." && $object != "..") 
				{
					...
					{
						if(!in_array($object, DownloadExplorer::getConfig('hidden_dirs')))
							$this->dirs[] = new Dir($object, $this->location);
					}
					else if(!in_array($object, DownloadExplorer::getConfig('hidden_files')))
		...
		{
			$DownloadExplorer->setErrorString("unable_to_read_dir");;
		}
	}
	...
	{
		if(DownloadExplorer::getConfig('calculate_space_level') <= 0)
			return;
		$ignore_files = array('..', '.');
		$start_dir = getcwd();
		$spaceUsed = $this->sum_dir($start_dir, $ignore_files, DownloadExplorer::getConfig('calculate_space_level'));
	...
	function sort()
	{
		if(is_array($this->files)){
			usort($this->files, "DownloadExplorer::cmp_".$this->sort_by);
			if($this->sort_as == "desc")
				$this->files = array_reverse($this->files);
		}
		
		if(is_array($this->dirs)){
			usort($this->dirs, "DownloadExplorer::cmp_name");
	...
	{
		$link = "?";
		if($switchVersion == true && DownloadExplorer::getConfig("mobile_enabled") == true)
		{
		...
		else if($this->mobile == true && DownloadExplorer::getConfig("mobile_enabled") == true && DownloadExplorer::getConfig("mobile_default") == false)
			$link .= "m&amp;";
		else if($this->mobile == false && DownloadExplorer::getConfig("mobile_enabled") == true && DownloadExplorer::getConfig("mobile_default") == true)
			$link .= "s&amp;";
		...
		if(isset($this->lang) && $this->lang != DownloadExplorer::getConfig("lang"))
			$link .= "lang=".$this->lang."&amp;";
	...
	function formatModTime($time)
	{
		...
		if(DownloadExplorer::getConfig("time_format") != null && strlen(DownloadExplorer::getConfig("time_format")) > 0)
			$timeformat = DownloadExplorer::getConfig("time_format");
	...
	{
		return DownloadExplorer::getLangString($stringName, $this->lang);
	}
	...
	{
		DownloadExplorer::setError($this->getString($stringName));
	}
...
<?php 
	if(DownloadExplorer::getConfig("thumbnails") == true && $this->mobile == false)
	{
?>
...
?>
<title><?php if(DownloadExplorer::getConfig('main_title') != null) print DownloadExplorer::getConfig('main_title'); ?></title>
...
<script>
  ...
  ga('create', 'UA-46079728-1', 'localhost');
  ga('send', 'pageview');
...
<div id="frame">
<?php
if(DownloadExplorer::getConfig('show_top') == true)
{
?>
<div id="top">
	<a href="<?php print $this->makeLink(false, false, null, null, null, ""); ?>"><span><?php if(DownloadExplorer::getConfig('main_title') != null) print DownloadExplorer::getConfig('main_title'); ?></span></a>
<?php 
if(DownloadExplorer::getConfig("secondary_titles") != null && is_array(DownloadExplorer::getConfig("secondary_titles")) && count(DownloadExplorer::getConfig("secondary_titles")) > 0 && $this->mobile == false)
{
	$secondary_titles = DownloadExplorer::getConfig("secondary_titles");
...
if($this->mobile == false && DownloadExplorer::getConfig("show_path") == true)
{
?>
...
if($this->files)
{
	...
	foreach ($this->files as $file)
	{
		...
		if(DownloadExplorer::getConfig('open_in_new_window') == true)
			print "target=\"_blank\"";
...
&nbsp<g:plusone></g:plusone>
</div>
<!-- END: Info area -->
<center>
<div class="fb-comments" data-href="" data-numposts="5" data-colorscheme="dark"></div>
</center>
</body>
</html>
<?php
...
$DownloadExplorer = new DownloadExplorer();
$DownloadExplorer->init();
